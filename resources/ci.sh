...
docker run -u `id -u $USER` -e TRAVIS_PULL_REQUEST=${TRAVIS_PULL_REQUEST} -e TRAVIS_TAG=${TRAVIS_TAG} -e TRAVIS_BRANCH=${TRAVIS_BRANCH} -e TRAVIS_COMMIT=${TRAVIS_COMMIT} -e SONAR_API_KEY=${SONAR_API_KEY} --rm -v `pwd`:/code naccyde/yall /code/resources/build.sh
